"""
EDCT — Encoder-Decoder Causal Transformer.

Introduced alongside CT (Melnychuk, Frauen & Feuerriegel, 2022, ICML) as an
encoder-decoder variant with the same adversarial domain-confusion
objective, separating encoder and autoregressive-decoder training into two
phases.

Not one of the four baselines the paper "Causal State-Space Model for
Causal Inference: Estimating Longitudinal Individual Treatment Effects"
reports as a standalone comparison row (see
ct.py's docstring for the paper's actual baseline list). Trained via the
same two-phase runnables/train_enc_dec.py script as CRN, though CRN uses
its own independent CRNEncoder/CRNDecoder classes (crn.py), not this file.

The code in this file and ct.py is adapted from the original CT authors'

"""
CT — Causal Transformer (primary baseline).

Melnychuk, Frauen & Feuerriegel (2022). "Causal Transformer for Estimating
Counterfactual Outcomes." ICML 2022.

The primary baseline reported in the paper "Causal State-Space Model for
Causal Inference: Estimating Longitudinal Individual Treatment Effects"
— the direct comparison point for CSSD, CSSPD,
CHSD, and CHSPD (see cssd.py). A three-stream self-attention model with
adversarial domain-confusion balancing; every proposed model in this repo
inherits its training loop, BRTreatmentOutcomeHead, and multi-input build_br
structure from this class (via SST — see sst.py) and only replaces the
attention blocks with SSM/hybrid blocks.

The code in this file and edct.py is adapted from the original authors'
public repository; see LICENSE for the retained copyright notice.
"""

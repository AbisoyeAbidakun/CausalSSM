...
class MixtureDensityOutcomeHead(nn.Module):
    """
    Gaussian-mixture outcome head for counterfactual uncertainty quantification.

    Replaces the point-estimate linear outcome layer in BRTreatmentOutcomeHead
    with a K-component Gaussian mixture, providing per-timestep mean and
    variance estimates over counterfactual outcomes.

    Used by CausalMixer as an optional extra head (use_mixture_head=True in
    config).  The main training loop still uses BRTreatmentOutcomeHead's point
    estimate for MSE loss compatibility; this head can be queried post-training
    or fine-tuned with an NLL objective.

    Parameters
    ----------
    br_size        : int  balanced-representation dimension
    dim_treatments : int
    dim_outcome    : int
    n_components   : int  number of mixture components (default 5)
    """
    ...
    def __init__(
        ...
        self.n = n_components
        self.dim_outcome = dim_outcome
        in_dim = br_size + dim_treatments
        self.mean_head = nn.Linear(in_dim, dim_outcome * n_components)
        self.var_head = nn.Linear(in_dim, dim_outcome * n_components)
        self.mix_head = nn.Linear(in_dim, n_components)

    def forward(self, br, treatments):
        """
        Args:
            br         : [..., br_size]
            treatments : [..., dim_treatments]
        Returns:
            means   : [..., dim_outcome, n_components]
            vars    : [..., dim_outcome, n_components]  (positive)
            weights : [..., n_components]               (sum to 1)
        """
        x = torch.cat([br, treatments], dim=-1)
        shape = x.shape[:-1]
        means = self.mean_head(x).view(*shape, self.dim_outcome, self.n)
        log_vars = self.var_head(x).view(*shape, self.dim_outcome, self.n)
        weights = F.softmax(self.mix_head(x), dim=-1)
        return means, log_vars.exp(), weights

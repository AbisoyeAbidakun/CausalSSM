...
def train_eval_factual(args: dict, train_f: Dataset, val_f: Dataset, orig_hparams: DictConfig, input_size: int, model_cls,
                       tuning_criterion='rmse', autoregressive=None, has_vitals=None,
                       bce_weights=None, projection_horizon=None):
    """
    ...
    :param tuning_criterion: 'rmse' or 'bce'
    :param autoregressive: Flag of including previous outcomes to modelling
    :param has_vitals: Flag of vitals in dataset
    :param bce_weights: Re-weight BCE if used
    :param projection_horizon: Range of tau-step-ahead prediction
    """
    model_kwargs = dict(autoregressive=autoregressive, has_vitals=has_vitals,
                        bce_weights=bce_weights, projection_horizon=projection_horizon)
    ...
    if model_cls.model_type == 'decoder':
        ...
        model = model_cls(new_params, encoder_r_size=encoder_r_size, **model_kwargs).to(torch.get_default_dtype())
    else:
        model = model_cls(new_params, **model_kwargs).to(torch.get_default_dtype())

    train_loader = DataLoader(train_f, shuffle=True, batch_size=new_params.model[model_cls.model_type]['batch_size'],
                              drop_last=True)
    trainer = Trainer(gpus=eval(str(new_params.exp.gpus))[:1] or None,
                      logger=None,
    ...
    if tuning_criterion == 'rmse':
        ...
        ray_train.report({'val_rmse_orig': val_rmse_orig, 'val_rmse_all': val_rmse_all})
    elif tuning_criterion == 'bce':
        val_bce_orig, val_bce_all = model.get_masked_bce(val_f)
        ray_train.report({'val_bce_orig': val_bce_orig, 'val_bce_all': val_bce_all})
    else:
...
class TimeVaryingCausalModel(LightningModule):
    ...
    def train_dataloader(self) -> DataLoader:
        sub_args = self.hparams.model[self.model_type]
        _nw = getattr(self.hparams.exp, 'num_workers', 4)
        return DataLoader(self.dataset_collection.train_f, shuffle=True, batch_size=sub_args['batch_size'],
                          drop_last=True, num_workers=_nw, persistent_workers=_nw > 0)
...
class BRCausalModel(TimeVaryingCausalModel):
    ...
    def training_step(self, batch, batch_ind, optimizer_idx=0):
        ...
        if optimizer_idx == 0:  # grad reversal or domain confusion representation update
            ...
            if self.balancing == 'grad_reverse':
                bce_loss = self.bce_loss(treatment_pred, batch['current_treatments'].to(torch.get_default_dtype()), kind='predict')
            elif self.balancing == 'domain_confusion':
                bce_loss = self.bce_loss(treatment_pred, batch['current_treatments'].to(torch.get_default_dtype()), kind='confuse')
    ...
    def test_step(self, batch, batch_ind, **kwargs):
        ...
        subset_name = self.trainer.test_dataloaders[0].dataset.subset_name

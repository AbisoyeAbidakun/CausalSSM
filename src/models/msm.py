...
Robins, Hernan & Brumback (2000). "Marginal Structural Models and Causal
Inference in Epidemiology." Epidemiology.

Available baseline infrastructure (runnable via runnables/train_msm.py)
but not one of the four baselines the paper "Causal State-Space Model for
Causal Inference: Estimating Longitudinal Individual Treatment Effects"
reports as a standalone comparison row — see
ct.py's docstring for the paper's actual reported baseline list (RMSN,
CRN, G-Net, CT). IPTW-weighted linear regression per horizon; no learned

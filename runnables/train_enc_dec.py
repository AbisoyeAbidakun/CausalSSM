...
def main(args: DictConfig):
    ...
    encoder_trainer = Trainer(**_get_trainer_kwargs(args), logger=mlf_logger, max_epochs=args.exp.max_epochs,
    ...
    if args.model.train_decoder:
        ...
        # PL 1.9 fix: pre-process decoder data BEFORE creating the decoder trainer.
        # In PL 1.9, LightningModule.trainer.setter propagates the trainer ref to all nn.Module children.
        # CRNDecoder stores self.encoder as a submodule, so decoder_trainer.strategy.connect(decoder)
        # overwrites encoder.trainer → decoder_trainer. Then prepare_data() calls
        # encoder.get_representations() → decoder_trainer.predict(encoder), which resets
        # decoder_trainer.state.stage to None, killing the TRAINING flag before fit_loop.run().
        # Pre-processing here sets dataset_collection.processed_data_decoder = True so prepare_data()
        # skips the predict call entirely and the TRAINING stage flag is never disturbed.
        if not getattr(dataset_collection, 'processed_data_decoder', False):
            dataset_collection.process_data_decoder(encoder)
        ...
        decoder_trainer = Trainer(**_get_trainer_kwargs(args), logger=mlf_logger, max_epochs=args.exp.max_epochs,

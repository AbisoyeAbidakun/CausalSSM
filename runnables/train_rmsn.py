...
def _cast_dataset_to_float32(dataset_collection):
    ...
    MPS does not support float64; numpy arrays from pandas default to float64 and are
    collated as float64 tensors by the DataLoader unless explicitly cast beforehand."""
    import numpy as np
...
def main(args: DictConfig):
    ...
    propensity_treatment_trainer = Trainer(**_get_trainer_kwargs(args),
                                           logger=mlf_logger,
    ...
    propensity_history_trainer = Trainer(**_get_trainer_kwargs(args),
                                         logger=mlf_logger,
                                         max_epochs=args.exp.max_epochs,
    ...
    # Pre-compute stabilised propensity weights BEFORE creating encoder_trainer.
    # RMSNEncoder stores propensity_treatment/history as nn.Module children; when
    # encoder_trainer.fit(encoder) is called, PL propagates encoder.trainer →
    # propensity networks, causing prepare_data() to call encoder_trainer.predict()
    # which corrupts encoder_trainer's training state before the fit loop starts.
    # By running prepare_data() here the propensity networks still own their original
    # trainers, the predict calls succeed, and sw_tilde_enc is cached in the dataset
    # so prepare_data() is a no-op inside encoder_trainer.fit().
    encoder.prepare_data()
    logger.info("Pre-computed RMSN encoder stabilised weights (sw_tilde_enc).")

    if args.model.encoder.tune_hparams:
        encoder.finetune(resources_per_trial=args.model.encoder.resources_per_trial)

    encoder_trainer = Trainer(**_get_trainer_kwargs(args),
                              logger=mlf_logger,
                              max_epochs=args.exp.max_epochs,
    ...
    # RMSNEncoder has no test_step — RMSE computed directly via get_normalised_masked_rmse
    ...
    if args.model.train_decoder:
        ...
        # Pre-compute decoder propensity weights before creating decoder_trainer.
        # RMSNDecoder stores encoder as child nn.Module; PL would propagate
        # decoder_trainer → encoder → encoder.trainer.predict() corrupts decoder_trainer state.
        decoder.prepare_data()
        logger.info("Pre-computed RMSN decoder stabilised weights (sw_tilde_dec).")

        if args.model.decoder.tune_hparams:
            decoder.finetune(resources_per_trial=args.model.decoder.resources_per_trial)

        decoder_trainer = Trainer(**_get_trainer_kwargs(args),
                                  logger=mlf_logger,
                                  max_epochs=args.exp.max_epochs,
        ...
        # RMSNDecoder has no test_step — RMSE computed directly via get_normalised_masked_rmse
